...
class ManagerBasedRLEnv(ManagerBasedEnv, gym.Env):
    ...
    def __init__(self, cfg: ManagerBasedRLEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.joint_position = [0.0,-1.57, 1.57, -1.57, -1.57,0.0] 
    ...
    def save_joint_values_to_csv(self,joint_values, file_path):
        # Convert tensor to list
        joint_values_list = joint_values.tolist()[0]
        
        # Save to CSV
        with open(file_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(joint_values_list)
        print(f"Joint values saved to {file_path}")

    #Return get joint values
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        for _ in range(self.cfg.decimation):
            ...
            # Extract joint_position
            joint_pose_tensor = current_state['articulation']['robot']['joint_position']
            self.joint_position = [round(val, 3) for val in joint_pose_tensor.tolist()[0]]


            # Print the joint_position
            #print("Joint position:", self.joint_position)

            # Path to save the CSV file
            #file_path = '/workspace/isaaclab/joint_value.csv'

            # Save the joint values to CSV
            #self.save_joint_values_to_csv(joint_position, file_path)
