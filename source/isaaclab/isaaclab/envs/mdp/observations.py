...
def read_pose():
    with open('/workspace/isaaclab/goal.csv', mode ='r')as file:
...
def generated_commands(env: ManagerBasedRLEnv, command_name: str) -> torch.Tensor:
    ...
    list_val = read_pose()
    print("From file",list_val)

    # Convert list to tensor and specify the device
    tensor_val = torch.tensor([list_val], device='cuda:0')
    print("Tensor value", tensor_val)
