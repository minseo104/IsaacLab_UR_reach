...
class PnpNode(Node):
	...
	# Random value function to generate values within given ranges
    def get_random_pose_values(self):
	    x = random.uniform(0.1, 0.5)  # First value between -0.5 and 0.5
	    y = random.uniform(0.1, 0.4)  # Second value between -0.4 and 0.4
	    z = random.uniform(0.1, 0.5)   # Third value between 0.1 and 0.5
	    return x, y, z
    ...
    def pick_and_place_sequence(self):
        while rclpy.ok():
            ...
            if self.pick_color == "red":
                ...
                x, y, z = self.get_random_pose_values()
