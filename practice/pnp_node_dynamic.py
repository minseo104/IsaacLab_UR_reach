...
class PnpNode(Node):
    ...
    def __init__(self):
        ...
        # Create a subscriber for /object_3d_pose topic
    ...
    def pose_callback(self, msg):
        if msg.z > 0.0:
            self.current_obj_distance = msg.z
    ...
    def pick_and_place_sequence(self):
        while rclpy.ok():
            ...
            self.publish_pose(0.436, 0.014, 0.2)
            time.sleep(1)

            self.width = 0.05

            self.get_logger().info(f'Current object distance: {self.current_obj_distance}')

            if (   self.current_obj_distance > self.trigger_distance ):
                self.get_logger().info('Object is within trigger distance, proceeding with pick and place...')
                ...
                if self.pick_color == "red":
                    ...
                    self.publish_pose(0.43, 0.014, z_pose)
                    time.sleep(delay)
                ...
                self.get_logger().warn('Object is not within trigger distance, skipping pick and place...')
                continue
